...
class EmailIntentBuilder private constructor(context: Context) {
    ...
    private val to = LinkedHashSet<String>()
    private val cc = LinkedHashSet<String>()
    private val bcc = LinkedHashSet<String>()
    ...
    private fun addRecipientQueryParameters(mailto: StringBuilder, field: String, recipients: Set<String>,
                                            hasQueryParameters: Boolean): Boolean {
    ...
    private fun checkEmail(email: String) {
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            throw IllegalArgumentException("Argument is not a valid email address (according to " + "Patterns.EMAIL_ADDRESS)")
        }
    }
    ...
    private fun checkNoLineBreaks(text: String) {
        ...
        if (containsCarriageReturn || containsLineFeed) {
            throw IllegalArgumentException("Argument must not contain line breaks")
        }
    }
    ...
    companion object {
        ...
        private fun <T> checkNotNull(`object`: T?): T {
            if (`object` == null) {
                throw IllegalArgumentException("Argument must not be null")
            }
        ...
        @VisibleForTesting
        internal fun encodeRecipient(recipient: String): String {
        ...
        @VisibleForTesting
        internal fun fixLineBreaks(text: String): String {
            return text.replace("\r\n".toRegex(), "\n").replace('\r', '\n').replace("\n".toRegex(), "\r\n")
        }
    }
}